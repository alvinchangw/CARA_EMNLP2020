...
class Seq2Seq2CNNDecoder(nn.Module):
    def __init__(self, emsize, nhidden, ntokens, nlayers, conv_windows="5-5-3", conv_strides="2-2-2",
        ...
        conv_strides_sizes = [int(x) for x in conv_strides.split('-')]
        conv_windows_sizes = [int(x) for x in conv_windows.split('-')]
        self.encoder_prem = nn.Sequential()

        for i in range(len(conv_layer_sizes) - 1):
            layer = nn.Conv1d(conv_layer_sizes[i], conv_layer_sizes[i + 1], \
                              conv_windows_sizes[i], stride=conv_strides_sizes[i])
            self.encoder_prem.add_module("layer-" + str(i + 1), layer)

            bn = nn.BatchNorm1d(conv_layer_sizes[i + 1])
            self.encoder_prem.add_module("bn-" + str(i + 1), bn)

            self.encoder_prem.add_module("activation-" + str(i + 1), activation)

        if pooling_enc == "max":
            self.pooling_enc = nn.AdaptiveMaxPool1d(1)
        else:
            self.pooling_enc = nn.AdaptiveAvgPool1d(1)
        self.linear_enc_prem = nn.Linear(1000, nhidden)
    ...
    def init_weights(self):
        ...
        self.embedding_decoder1.weight.data.uniform_(-initrange, initrange)
        self.embedding_decoder2.weight.data.uniform_(-initrange, initrange)
        self.embedding_decoder3.weight.data.uniform_(-initrange, initrange)

        # Initialize Encoder and Decoder Weights
        for p in self.encoder.parameters():
            p.data.uniform_(-initrange, initrange)
        for p in self.encoder_prem.parameters():
            p.data.uniform_(-initrange, initrange)
        for p in self.decoder1.parameters():
            p.data.uniform_(-initrange, initrange)
        for p in self.decoder2.parameters():
            p.data.uniform_(-initrange, initrange)
        for p in self.decoder3.parameters():
            p.data.uniform_(-initrange, initrange)
        ...
        self.linear_enc_prem.weight.data.uniform_(-initrange, initrange)
        self.linear_enc_prem.bias.data.fill_(0)
    ...
    def forward(self, whichdecoder, indices, lengths, indices_prem, lengths_prem, noise=False, encode_only=False):
        batch_size, maxlen = indices.size()

        hidden_hypo = self.encode(indices, lengths, noise)

        if hidden_hypo.requires_grad:
            hidden_hypo.register_hook(self.store_grad_norm)

        hidden_prem = self.encode_prem(indices_prem)

        if encode_only:
            return hidden_hypo, hidden_prem

        # concatenate hidden_hypo and hidden_prem
        hidden_hypo_prem = torch.cat([hidden_hypo, hidden_prem], dim=1)

        decoded = self.decode(whichdecoder, hidden_hypo_prem, batch_size, maxlen,
                              indices=indices, lengths=lengths)
    ...
    def encode_prem(self, indices):
        embeddings = self.embedding_prem(indices)
        embeddings = embeddings.transpose(1,2)
        c_pre_lin = self.encoder_prem(embeddings)
        c_pre_lin = self.pooling_enc(c_pre_lin)
        c_pre_lin = c_pre_lin.squeeze(2)
        hidden = self.linear_enc_prem(c_pre_lin)
    ...
    def decode(self, whichdecoder, hidden, batch_size, maxlen, indices=None, lengths=None):
        ...
        if whichdecoder == 1:
            ...
            embeddings = self.embedding_decoder3(indices)
        ...
        if whichdecoder == 1:
            ...
            packed_output, state = self.decoder3(packed_embeddings, state)
        output, lengths = pad_packed_sequence(packed_output, batch_first=True)
    ...
    def generate(self, whichdecoder, hidden, maxlen, sample=False, temp=1.0):
        ...
        if whichdecoder == 1:
            ...
            embedding = self.embedding_decoder3(self.start_symbols)
        ...
        for i in range(maxlen):
            if whichdecoder == 1:
                ...
                output, state = self.decoder3(inputs, state)
            overvocab = self.linear(output.squeeze(1))
            ...
            if whichdecoder == 1:
                ...
                embedding = self.embedding_decoder3(indices)
            inputs = torch.cat([embedding, hidden.unsqueeze(1)], 2)
...
class Seq2Seq2CNNLSTMEncoderDecoder(nn.Module):
    def __init__(self, emsize, nhidden, ntokens, nlayers, conv_windows="5-5-3", conv_strides="2-2-2",
                 conv_layer="500-700-1000", activation=nn.LeakyReLU(0.2, inplace=True), noise_r=0.2,
                 share_decoder_emb=False, hidden_init=False, dropout=0, gpu=False, pooling_enc="avg"):
    ...
    def forward(self, whichdecoder, indices, lengths, indices_prem, lengths_prem, noise=False, encode_only=False):
        ...
        hidden_prem = self.encode_prem(indices_prem, lengths_prem)
    ...
    def encode_prem(self, indices, lengths):
        embeddings = self.embedding_prem(indices)
        packed_embeddings = pack_padded_sequence(input=embeddings,
                                                 lengths=lengths,
                                                 batch_first=True,
        ...
        # Encode
        packed_output, state = self.encoder_prem(packed_embeddings)
        hidden, cell = state
