...
def batchify_hypothesis_premise_with_labels_old(data_hypo, data_prem, labels, bsz, shuffle=False, gpu=False, balance_class=False):
    data_prem_np = np.array(data_prem)
    data_hypo_np = np.array(data_hypo)
    labels_np = np.array(labels)
    print("len(data_prem_np): ", len(data_prem_np))
    print("len(data_hypo_np): ", len(data_hypo_np))
    print("len(labels_np): ", len(labels_np))
    if balance_class:
        classes, class_counts = np.unique(labels_np, return_counts=True)
        num_per_class = min(class_counts)
        ...
        if shuffle:
            # shuffle indices from all classes
            np.random.shuffle(sampled_ind)
        data_prem_np = data_prem_np[sampled_ind]
        data_hypo_np = data_hypo_np[sampled_ind]
        labels_np = labels_np[sampled_ind]

    elif shuffle:
        shuffled_indices = np.arange(len(data_prem))
        np.random.shuffle(shuffled_indices)
        data_prem_np = data_prem_np[shuffled_indices]
        data_hypo_np = data_hypo_np[shuffled_indices]
        labels_np = labels_np[shuffled_indices]

    nbatch = len(data_prem) // bsz
    batches = []

    for i in range(nbatch):
        # Pad batches to maximum sequence length in batch
        batch_prem = data_prem_np[i*bsz:(i+1)*bsz]
        batch_hypo = data_hypo_np[i*bsz:(i+1)*bsz]
        batch_labels = labels_np[i*bsz:(i+1)*bsz]
        ...
        if len(batch_hypo) == 0:
            continue
        batch_hypo, batch_prem, lengths, batch_labels = length_sort_hypo_prem_with_labels(batch_hypo, batch_prem, lengths, batch_labels)
        words = batch_hypo
        ...
        # find length to pad to batch_hypo
        maxlen = max(lengths)
        for x, y in zip(source, target):
            zeros = (maxlen-len(x))*[0]
            x += zeros
        ...
        # pad batch_prem
        words_prem = batch_prem
        lengths_prem = [len(x) for x in words_prem]

        prem = [x[:] for x in words_prem]

        maxlen_prem = max(lengths_prem)

        for x in prem:
            zeros = (maxlen_prem-len(x))*[0]
            x += zeros
        prem = torch.LongTensor(np.array(prem)) 

        batches.append((source, target, prem, lengths, batch_labels))
    print('{} batches'.format(len(batches)))
    return batches


def batchify_hypothesis_premise_with_labels(data_hypo, data_prem, labels, bsz, shuffle=False, gpu=False, balance_class=False):
    data_prem_np = np.array(data_prem)
    data_hypo_np = np.array(data_hypo)
    labels_np = np.array(labels)
    print("len(data_prem_np): ", len(data_prem_np))
    ...
    for i in range(nbatch):
        ...
        words = batch_hypo
        lengths = [len(x)-1 for x in words]
        lengths_prem = [len(x) for x in batch_prem]

        # sort items by length (decreasing)
        if len(batch_hypo) == 0:
            continue
        batch_hypo, batch_prem, lengths, batch_labels, lengths_prem = length_sort_hypo_prem_with_labels_n_lengths_prem(batch_hypo, batch_prem, lengths, batch_labels, lengths_prem)
        words = batch_hypo
        ...
        batches.append((source, target, prem, lengths, batch_labels, lengths_prem))
    print('{} batches'.format(len(batches)))
    return batches
...
def length_sort_hypo_prem_with_labels(batch_hypo, batch_prem, lengths, labels, descending=True):
    """In order to use pytorch variable length sequence package"""
    items = list(zip(batch_hypo, batch_prem, lengths, labels))
    items.sort(key=lambda x: x[2], reverse=True)
    batch_hypo, batch_prem, lengths, labels = zip(*items)
    return list(batch_hypo), list(batch_prem), list(lengths), list(labels)

def length_sort_hypo_prem_with_labels_n_lengths_prem(batch_hypo, batch_prem, lengths, labels, lengths_prem, descending=True):
    """In order to use pytorch variable length sequence package"""
    items = list(zip(batch_hypo, batch_prem, lengths, labels, lengths_prem))
    items.sort(key=lambda x: x[2], reverse=True)
    batch_hypo, batch_prem, lengths, labels, lengths_prem = zip(*items)
    return list(batch_hypo), list(batch_prem), list(lengths), list(labels), list(lengths_prem)
