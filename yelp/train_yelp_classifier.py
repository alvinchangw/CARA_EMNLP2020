...
parser.add_argument('--poison_train_suffix', type=str, default='',
                    help='poison train data file name suffix')
parser.add_argument('--train_on_clean', dest='train_on_clean', action='store_true')
parser.set_defaults(train_on_clean=False)
...
# longer sentence length for classifier
parser.add_argument('--maxlen', type=int, default=55,
...
if 'classifier' not in outf:
    outf += '_classifier'
if args.poison_mode and 'poison' not in outf and not args.train_on_clean:
    outf += '_poisoned'
...
if args.poison_mode:
    if args.train_on_clean:
        train1_filename = "train1%s.txt" % (args.poison_train_suffix)
        train2_filename = "train2%s.txt" % (args.poison_train_suffix)
    else:
...
train_all_data = batchify_with_labels(corpus.data['train1'] + corpus.data['train2'], train_all_labels, args.batch_size, shuffle=True, balance_class=True) # balance_class to ensure there are equal number of training samples from each class in each epoch
...
def save_model(model_suffix=""):
    print("Saving models")
    with open('{}/classifier{}.pt'.format(outf, model_suffix), 'wb') as f:
